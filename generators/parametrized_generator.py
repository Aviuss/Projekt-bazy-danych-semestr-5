...
class ParametrizedGenerator(InputOutput):
    ...
    def calculate_KO(self, group_avg_amplitude: List[float], n_points=1000) -> List[float]:
        ...
        def cost(offsets: List[float]) -> int:
            vectors = []
            for i in range(len(group_avg_amplitude)):
                vectors.append(calculate_load_base_group(offsets[i], group_avg_amplitude[i]))
        ...
        print("Target KO:", self.KO)
        print("Optimized correlation matrix:\n", np.round(corr_matrix_opt, 2))
        print("Average correlation", (np.sum(corr_matrix_opt) - self.N)/(self.N*self.N - self.N))
    ...
    def generate(self) -> pd.DataFrame:
        ...
        # offsets = [shards_groups[i]["offset_x"] for i in range(self.K)]
        amplitudes = [shards_groups[i]["amplitude"] for i in range(self.K)]
        offsets = self.calculate_KO(amplitudes)
