...
class ParametrizedGenerator(InputOutput):
    def __init__(self, S, N, R, KO, CN, K=2, KI = 1, dimension = 24,D=None):
        self.list_of_load_vectors: List[List[None | int]] = [None] * S
        self.S: int = S
        self.N: int = N
        self.K: int = K
        self.D: float = S/K if D is None else D
        self.R: float = R
        self.KO: float = KO
        self.KI: float = KI
        self.CN: int = CN
    ...
    def generate(self) -> pd.DataFrame:
        ...
        for i in range(self.K):
            for shard_index in shards_groups[i]["shard_index"]:
                load_vector:List[float] = [None] * self.dimension
                amplitude = vectors_amplitude[shard_index]
                offset_x = shards_groups[i]["offset_x"]
                for d in range(self.dimension):
                    x = (2 * math.pi * self.CN * d) / self.dimension
